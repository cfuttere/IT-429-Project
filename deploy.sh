...
EOF
    ...
    if [ "$DEPLOYMENT_TYPE" == "pdf2pdf" ]; then
        ...
        SOURCE_VERSION="Kwantrengss"  # Use pdf2html-subtree since buildspec only exists there
        BUILDSPEC_FILE="buildspec-unified.yml"
    else
        BUILD_IMAGE="aws/codebuild/amazonlinux2-x86_64-standard:5.0"
        COMPUTE_TYPE="BUILD_GENERAL1_LARGE"
        PRIVILEGED_MODE="true"
        SOURCE_VERSION="Kwantrengss"
        BUILDSPEC_FILE="buildspec-unified.yml"
    fi
...
deploy_ui() {
    ...
    if ! git clone -b Kwantreng https://github.com/cfuttere/IT-429-Project-pdf2html-UI "$UI_TEMP_DIR" 2>/dev/null; then
...
GITHUB_URL="https://github.com/cfuttere/IT-429-Project.git"
